...
const TradeMessageModal: React.FC<Props> = ({
  ...
  return (
    <div className="tradeModalOverlay" onClick={onClose}>
      <div className="tradeModalCard" onClick={stop} role="dialog" aria-modal="true">
        <div className="tradeModalHeader">
          <div className="tradeModalTitleWrap">
            <h2 className="tradeModalTitle">{t('tradeModal.title', 'Send trade message')}</h2>
        ...
        <div className="tradeModalBody">
          <div className="tradeModalLeft">
            <div className="tradeModalImageWrap">
              <img src={cardImage} alt={t('tradeModal.cardAlt', 'Card')} className="tradeModalImage" />
            </div>
          </div>

          <div className="tradeModalRight">
            <label className="tradeModalLabel">{t('tradeModal.chooseUser', 'User you want to trade with')}</label>

            <TradeOwnerSelect
              value={selectedOwner}
              options={ownerOptions}
              onChange={onOwnerChange}
              placeholder={t('tradeModal.chooseUser', 'User you want to trade with')}
            />

            <label className="tradeModalLabel">{t('tradeModal.offerLabel', 'Message / card you offer')}</label>

            <textarea
              className="tradeModalTextarea"
              value={note}
              onChange={(e) => onNoteChange(e.target.value)}
              placeholder={t('tradeModal.offerPlaceholder', 'Describe the card you offer or the trade terms…')}
              rows={5}
            />

            <div className="tradeModalActions">
              <button type="button" className="btn-gray-small" onClick={onClose}>
                {t('common.close', 'Close')}
              </button>

              <button type="button" className="btn-blue-small" onClick={onSend} disabled={!canSend}>
            ...
            {!canSend && (
              <p className="tradeModalHint tradeModalHint--soft">
                {t('tradeModal.hintFill', 'Select a user and write a message to send the request.')}
              </p>
            )}
          </div>
        </div>
