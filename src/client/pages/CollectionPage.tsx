...
import { SlidersHorizontal, ArrowUpDown, Grid2X2, Grid3X3, ArrowUp, ArrowDown, Tag, X, ChevronDown, Check } from "lucide-react";

const RARITY_ORDER = [
  "Common","Uncommon","Rare","Holo Rare","Rare Holo","Double rare","ACE SPEC Rare","Amazing Rare",
  "Illustration rare","Special illustration rare","Ultra Rare","Holo Rare V","Holo Rare VMAX","Holo Rare VSTAR",
  "Shiny rare","Shiny Ultra Rare","Shiny rare V","Shiny rare VMAX","Radiant Rare","Hyper rare","Mega Hyper Rare",
  "Secret Rare","Rare PRIME","Rare Holo LV.X","LEGEND","Full Art Trainer","Classic Collection","Black White Rare",
  "Crown","One Diamond","Two Diamond","Three Diamond","Four Diamond","One Star","Two Star","Three Star",
  "One Shiny","Two Shiny","None"
];
...
const SelectPopover: React.FC<{
  ...
  searchable?: boolean;
  maxHeight?: number;
}> = ({ value, onChange, options, placeholder, searchable = true, maxHeight = 420 }) => {
  ...
  return (
    <div className="selectWrap" ref={wrapRef}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
      >
        <span className={`selectValue ${value ? "" : "isPlaceholder"}`}>{label}</span>
      ...
      {open && (
        ...
        >
          ...
          <div className={`selectList ${useTwoColumns ? "selectList--twoCol" : ""}`}>
            {filtered.length === 0 ? (
...
const CollectionPage: React.FC = () => {
  ...
  const rarities = useMemo(() => {
    ...
    const orderedFromCanonical = RARITY_ORDER
      .filter((r) => presentMap.has(r.toLowerCase()))
      .map((r) => presentMap.get(r.toLowerCase())!);

    const extras = present
      .filter((p) => !RARITY_ORDER.some((r) => r.toLowerCase() === p.toString().toLowerCase()))
  ...
  useEffect(() => {
    setPage(1);
  }, [query, selectedSet, selectedRarity, selectedType, selectedSort, sortDir, view]);
  ...
  const setOptions: SelectOption[] = useMemo(
    () => [{ value: "", label: t("common.all") || "Todos" }, ...sets.map((s) => ({ value: s, label: s }))],
    [sets, t]
  );

  const rarityOptions: SelectOption[] = useMemo(
    () => [{ value: "", label: t("common.all") || "Todas" }, ...rarities.map((r) => ({ value: r, label: r }))],
    [rarities, t]
  );

  const typeOptions: SelectOption[] = useMemo(
    () => [{ value: "", label: t("common.all") || "Todos" }, ...types.map((tp) => ({ value: tp, label: tp }))],
    [types, t]
  );
      ...
      <main className="collectionMain">
          ...
          <div className="toolbarRightGroup">
              ...
              {filtersOpen && (
                <div className="panel panel--pro">
                  <div className="panelHeader">
                    <div className="panelHeaderTitle">{t("common.filter") || "Filtrar"}</div>
                    <button className="panelCloseBtn" onClick={() => setFiltersOpen(false)} type="button" aria-label="Cerrar">
                      <X size={18} />
                    </button>
                  </div>
                  ...
                  <div className="panelFooter panelFooter--sticky">
                    <button className="btnLite btnLite--ghost" onClick={clearFilters} type="button">
                      {t("common.clear") || "Limpiar"}
                    </button>
                    <button className="btnPrimary" onClick={() => setFiltersOpen(false)} type="button">
                      {t("common.done") || "Listo"}
                    </button>
                  </div>
                </div>
            ...
            <div className="panelWrap" ref={sortRef}>
              ...
              {sortOpen && (
                <div className="panel panel--pro">
                  <div className="panelHeader">
                    <div className="panelHeaderTitle">{t("collection.sort") || "Ordenar"}</div>
                    <button className="panelCloseBtn" onClick={() => setSortOpen(false)} type="button" aria-label="Cerrar">
                      <X size={18} />
                    </button>
                  </div>
                  ...
                  <div className="panelBody">
                    ...
                    >
                      ...
                      {sortDir === "asc" ? (t("common.asc") || "Ascendente") : (t("common.desc") || "Descendente")}
                    </button>
                  </div>

                  <div className="panelFooter panelFooter--sticky">
                    <button className="btnLite btnLite--ghost" onClick={() => { setSelectedSort(""); setSortDir("asc"); }} type="button">
                      {t("common.clear") || "Limpiar"}
                    </button>
                    <button className="btnPrimary" onClick={() => setSortOpen(false)} type="button">
                      {t("common.done") || "Listo"}
                    </button>
                  </div>
                </div>
        ...
        <div className="collectionContent">
          {loading ? (
            ...
            <section className={`cardsGrid ${view === "dense" ? "isDense" : "isComfortable"}`}>
              {pageItems.map((c: any) => {
                const isFlipped = flippedId === c.id;
                const forTrade = (optimisticTrades[c.id] ?? c.forTrade) ? true : false;
                ...
                return (
                  <article key={c.id} className="cardTileBig">
                    <div
                      ...
                      onMouseEnter={async () => { setFlippedId(c.id); await ensureDetail(c); }}
                     ...
                     <div className="flipFace flipBack">
                      <button
                        ...
                        onClick={async (e) => {
                          ...
                          const ok = await api.updateUserCard(u.username || u.id, c.id, { forTrade: next });
                          if (!ok) {
                      ...
                      <div className="backBody">
                        <div className="backTitle" title={c.name}>{c.name}</div>

                        <div className="backAttrGrid">
                          <div className="backAttrBox">
                            <span className="backAttrLabel">{t("common.rarity")}</span>
                            <span className="backAttrValue">{c.rarity || "—"}</span>
                          </div>

                          <div className="backAttrBox">
                            <span className="backAttrLabel">{t("common.set")}</span>
                            <span className="backAttrValue">{c.set || c.series || "—"}</span>
                          </div>
                        </div>

                        <div className="backSection">
                          <span className="backSectionTitle">{t("common.illustrator")}</span>
                          <span className="backSectionValue">
                            {(d && (d.illustrator || d.artist)) || c.illustrator || "—"}
                          </span>
                        </div>
                        ...
                        <div className="backPrice">
                          {d ? (() => {
                            ...
                            <div className="loadingTiny">{t("common.loadingPrices")}</div>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="cardCaption" title={c.name}>{c.name}</div>
                  </article>
                );
              })}
            </section>
        ...
        <div className="pagerZone">
          <div className="pager">
            ...
            <div className="pagerInfo">{page} / {totalPages}</div>
