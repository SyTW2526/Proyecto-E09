    ...
    try {
      ...
      const setSideCard = (
        ...
        if (!trade[key] || trade[key].length === 0) {
          trade[key] = [
            { userCardId: new mongoose.Types.ObjectId(userCardId) },
          ];
        } else {
      ...
      if (isInitiator && trade.initiatorAccepted) {
        return res
          .status(400)
          .send({ error: 'Ya has aceptado este intercambio' });
      }
      ...
      if (receiverCard.userId.toString() !== trade.receiverUserId.toString()) {
        return res.status(400).send({
      ...
      try {
        if (trade.requestId) {
          const reqDoc: any = await TradeRequest.findById(
            trade.requestId
          ).lean();
        ...
        console.warn('Error comprobando carta solicitada en TradeRequest:', e);
      }
