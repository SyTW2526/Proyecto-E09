  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!receiverIdentifier) {
        return res.status(400).send({ error: 'receiverIdentifier es obligatorio' });
      }
      ...
      if (!receiver) return res.status(404).send({ error: 'Usuario destino no encontrado' });

      if (receiver._id.equals(me._id)) {
        return res.status(400).send({ error: 'No puedes enviarte una solicitud a ti mismo' });
      }
      ...
      if (isQuick && offeredUserCardId) {
        const offeredUC: any = await UserCard.findById(offeredUserCardId).populate('cardId');
        if (!offeredUC) return res.status(404).send({ error: 'La carta ofrecida (UserCard) no existe' });

        if (offeredUC.userId.toString() !== me._id.toString()) {
          return res.status(400).send({ error: 'La carta ofrecida no pertenece al usuario emisor' });
        }
      ...
      if (isQuick && targetUserCardId) {
        ...
        if (!targetUC) return res.status(404).send({ error: 'La carta objetivo (UserCard) no existe' });
        if (targetUC.userId.toString() !== receiver._id.toString()) {
          return res.status(400).send({ error: 'La carta objetivo no pertenece al usuario receptor' });
        }
      }
      ...
      res.status(500).send({ error: error.message ?? 'Error creando solicitud' });
    }
  }
);
...
tradeRequestRouter.get('/trade-requests/received/:userId', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(500).send({ error: error.message ?? 'Error obteniendo solicitudes' });
  }
});
...
tradeRequestRouter.post('/trade-requests/:id/open-room', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const currentUserId = req.userId;

    const request: any = await TradeRequest.findById(id);
    if (!request) return res.status(404).send({ error: 'Solicitud no encontrada' });

    if (request.to.toString() !== currentUserId?.toString()) {
      return res.status(403).send({ error: 'No puedes crear sala para esta solicitud' });
    }

    if (request.status !== 'pending') {
      return res.status(400).send({ error: 'La solicitud ya no está pendiente' });
    }
    ...
    return res.status(500).send({ error: error.message ?? 'Error creando sala' });
  }
});
...
tradeRequestRouter.post('/trade-requests/:id/reject', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (request.to.toString() !== currentUserId?.toString()) {
      return res.status(403).send({ error: 'No puedes rechazar esta solicitud' });
    }
    ...
    return res.status(500).send({ error: error.message ?? 'Error rechazando solicitud' });
  }
});
...
tradeRequestRouter.post('/trade-requests/:id/accept', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (request.to.toString() !== currentUserId?.toString()) {
      return res.status(403).send({ error: 'No puedes aceptar esta solicitud' });
    }
    ...
    if (request.offeredUserCardId) offeredUC = await UserCard.findById(request.offeredUserCardId);
    if (!offeredUC) {
    ...
    if (request.targetUserCardId) targetUC = await UserCard.findById(request.targetUserCardId);
    if (!targetUC) {
    ...
    if (!offeredUC || !targetUC) {
      return res.status(404).send({
        error: 'No se han encontrado las cartas en la colección para completar el intercambio rápido',
      });
    }
    ...
    const offeredP = typeof request.offeredPrice === 'number' ? request.offeredPrice : null;
    const targetP = typeof request.targetPrice === 'number' ? request.targetPrice : null;
      ...
      if (max > 0) {
        ...
        if (diffRatio > MAX_DIFF) {
          return res.status(400).send({
            ...
            details: { offeredPrice: offeredP, targetPrice: targetP, diffRatio },
          });
        }
      }
    }
    ...
    const transferOneCopy = async (sourceUserCard: any, targetUserId: mongoose.Types.ObjectId) => {
      const { cardId, pokemonTcgId, condition, collectionType, estimatedValue, isPublic } = sourceUserCard;
    ...
    return res.send({ message: 'QUICK_TRADE_COMPLETED', request, tradeId: trade._id });
  } catch (error: any) {
    console.error('Error aceptando solicitud:', error);
    return res.status(500).send({ error: error.message ?? 'Error aceptando solicitud' });
  }
});
...
tradeRequestRouter.delete('/trade-requests/:id/cancel', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (request.from.toString() !== currentUserId?.toString()) {
      return res.status(403).send({ error: 'No puedes cancelar esta solicitud' });
    }
    ...
    return res.status(500).send({ error: error.message ?? 'Error cancelando solicitud' });
  }
});
