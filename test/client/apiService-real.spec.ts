...
describe('apiService - real execution with mocked fetch', () => {
  ...
  it('fetchFeaturedCards returns data array on success', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/cards/featured');
  });
  ...
  it('getUserWishlist composes results and fetches cached missing cards', async () => {
    ...
    (global.fetch as any)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ cards: [{ _id: 'uc1', pokemonTcgId: 'sv01-1', forTrade: false }] }),
      })
      // cached card fetch
      .mockResolvedValueOnce({ ok: true, json: async () => ({ card: { name: 'Bulbasaur', images: { large: 'img.png' }, pokemonTcgId: 'sv01-1' } }) });
  ...
  it('updateUserCard sends PATCH and returns boolean', async () => {
    (global.fetch as any).mockResolvedValueOnce({ ok: true });
    const ok = await apiService.updateUserCard('user1', 'card1', { forTrade: true });
    expect(ok).toBe(true);
  });
});
