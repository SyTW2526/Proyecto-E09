...
describe('collectionSlice - reducer (pruebas en español)', () => {
  ...
  it('pone loading al iniciar fetch (pending)', () => {
    const next = collectionReducer(initialState, { type: fetchUserCollection.pending.type } as any);
  ...
  it('almacena cartas cuando fetch es exitoso (fulfilled)', () => {
    ...
    const next = collectionReducer(initialState, { type: fetchUserCollection.fulfilled.type, payload } as any);
    expect(next.loading).toBe(false);
    expect(next.cards).toEqual(payload);
  });

  it('gestiona error en fetch (rejected) con mensaje por defecto', () => {
    const action: any = { type: fetchUserCollection.rejected.type, error: { message: 'fallo' } };
  ...
  it('elimina carta al recibir removeFromCollection.fulfilled', () => {
    const stateWith: any = { ...initialState, cards: [{ id: 'a' }, { id: 'b' }] };
    const next = collectionReducer(stateWith as any, { type: removeFromCollection.fulfilled.type, payload: 'a' } as any);
